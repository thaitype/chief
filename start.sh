...
#
# Usage: ./start.sh <milestone-name> [max-iterations]
# Example: ./start.sh milestone-1
#          ./start.sh milestone-1 20

set -e

MILESTONE=${1:?"Usage: ./start.sh <milestone-name> [max-iterations]"}
MAX_ITERATIONS=${2:-10}
...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  OUTPUT=$(claude --dangerously-skip-permissions -p "@CLAUDE.md @$CHIEF_DIR/_rules @$GOAL_DIR @$MILESTONE_DIR/_contract @$TODO_FILE \
